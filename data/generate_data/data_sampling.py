# sample_tasks.py
# A unified, configuration-driven script for sampling RelBench tasks.
...
#                               TASKS CONFIGURATION
# ==============================================================================
# This dictionary is the single source of truth for all sampling tasks.
# Each entry defines the parameters required to sample a specific task.
#
# Parameters:
#   - parquet_id_column: The name of the column in the task's .parquet files
#                        that holds the entity ID.
#   - jsonl_id_key: The name of the key in the source .jsonl file that holds
#                   the entity ID.
#   - source_jsonl_file: The name of the .jsonl file generated by build_json.py
#                        that contains the full graph data for the entity.
#   - samples: A tuple specifying the number of samples for (train, val, test).
# ==============================================================================
TASKS_CONFIG = {
    # --- Regression Tasks ---
    ...
    # Note: 'site-success' requires a facility-centric JSONL, which we assume
    # is not built by default. This task is excluded unless that file exists.
    # "rel-trial/tasks/site-success": { ... }
    ...
    "rel-event/tasks/user-repeat": {
        ...
        "jsonl_id_key": "user_id",
        "source_jsonl_file": "rel-event-user.jsonl",
        "samples": (3842, 1000, 1000), # Using full train set as it's small
    },
    "rel-event/tasks/user-ignore": {
        "parquet_id_column": "user",
        "jsonl_id_key": "user_id",
        "source_jsonl_file": "rel-event-user.jsonl",
        "samples": (19239, 10000, 10000), # Using full train set
    },
    "rel-f1/tasks/driver-dnf": {
        "parquet_id_column": "driverId",
        "jsonl_id_key": "driverId",
        "source_jsonl_file": "rel-f1-driver.jsonl",
        "samples": (11411, 5000, 5000), # Using full train set
    },
    "rel-f1/tasks/driver-top3": {
        "parquet_id_column": "driverId",
        "jsonl_id_key": "driverId",
        "source_jsonl_file": "rel-f1-driver.jsonl",
        "samples": (1353, 1000, 1000), # Using full train set
    },
    ...
    "rel-trial/tasks/study-outcome": {
        ...
        "jsonl_id_key": "nct_id",
        "source_jsonl_file": "rel-trial-study.jsonl",
        "samples": (11994, 5000, 5000), # Using full train set
    },
}

def sample_single_task(task_name: str, config: Dict[str, Any], base_path_map: Dict[str, Path]):
    """
    Executes the sampling process for a single task based on its configuration.
    """
    ...
    # --- 2. Sample Parquet Files and Collect IDs ---
    ...
    # --- 3. Index and Filter Source JSONL File ---
